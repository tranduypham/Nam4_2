...
Namespace CopyBookmarkContent
	Partial Public Class Form1
		...
		Private Sub button1_Click(ByVal sender As Object, ByVal e As EventArgs) Handles button1.Click
			...
			Dim bookmark As Bookmark = doc.Bookmarks("Test")
			Dim docObj As DocumentObject = Nothing
			...
			If (TryCast(bookmark.BookmarkStart.Owner, Paragraph)).IsInCell Then
				docObj = bookmark.BookmarkStart.Owner.Owner.Owner.Owner
			Else
				docObj = bookmark.BookmarkStart.Owner
			End If
			Dim startIndex As Integer = doc.Sections(0).Body.ChildObjects.IndexOf(docObj)
			If (TryCast(bookmark.BookmarkEnd.Owner, Paragraph)).IsInCell Then
				docObj = bookmark.BookmarkEnd.Owner.Owner.Owner.Owner
			Else
				docObj = bookmark.BookmarkEnd.Owner
			End If
			Dim endIndex As Integer = doc.Sections(0).Body.ChildObjects.IndexOf(docObj)

			'Get the start/end index of the bookmark object on the paragraph.
			Dim para As Paragraph = TryCast(bookmark.BookmarkStart.Owner, Paragraph)
			Dim pStartIndex As Integer = para.ChildObjects.IndexOf(bookmark.BookmarkStart)
			para = TryCast(bookmark.BookmarkEnd.Owner, Paragraph)
			Dim pEndIndex As Integer = para.ChildObjects.IndexOf(bookmark.BookmarkEnd)

			'Get the content of current bookmark and copy.
			Dim [select] As New TextBodySelection(doc.Sections(0).Body, startIndex, endIndex, pStartIndex, pEndIndex)
			Dim body As New TextBodyPart([select])
			For i As Integer = 0 To body.BodyItems.Count - 1
